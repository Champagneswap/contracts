...
describe("WineMakerV2", function () {
  before(async function () {
    await prepare(this, ["WineMaker", "ChampagneToken", "ERC20Mock", "WineMakerV2", "RewarderMock", "RewarderBrokenMock"])
    await deploy(this, [["brokenRewarder", this.RewarderBrokenMock]])
  })

  beforeEach(async function () {
    await deploy(this, [["cham", this.ChampagneToken]])

    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(10)]],
      ["chef", this.WineMaker, [this.cham.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.WineMakerV2, [this.chef.address, this.cham.address, 1]],
  ...
  describe("PendingCham", function () {
    it("PendingCham should equal ExpectedCham", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedCham = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingCham = await this.chef2.pendingCham(0, this.alice.address)
      expect(pendingCham).to.be.equal(expectedCham)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accChamPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of CHAM and reward", async function () {
      ...
      await this.chef2.harvestFromWineMaker()
      let log2 = await this.chef2.withdraw(0, getBigNumber(1), this.alice.address)
      let expectedCham = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedCham)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.cham.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedCham)
    })
    ...
    it("Harvest for CHAM-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedCham)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.cham.balanceOf(this.alice.address)).to.be.equal(expectedCham)
    })
  })
