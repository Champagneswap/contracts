...
describe("MiniChefV2", function () {
  before(async function () {
    await prepare(this, ["MiniChefV2", "ChampagneToken", "ERC20Mock", "RewarderMock", "RewarderBrokenMock"])
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MiniChefV2, [this.cham.address]],
    ...
    await this.cham.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setChamPerSecond("10000000000000000")
  ...
  describe("PendingCham", function () {
    it("PendingCham should equal ExpectedCham", async function () {
      ...
      let expectedCham = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingCham = await this.chef.pendingCham(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef.poolInfo(0)).accChamPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of CHAM and reward", async function () {
      ...
      let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
      let expectedCham = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedCham)
