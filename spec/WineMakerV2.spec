/*
 * This is a specification file for WineMakerV2's formal verification
 * using the Certora prover.
 *
 * Run this file with scripts/_runWineMakerV2.sh
 */
...
using DummyCHAM as champagneToken
...
methods {
	...
	poolInfoAccChamPerShare(uint256 pid) returns (uint128) envfree
	...
	// CHAM token
	CHAM() returns (address) envfree

	// Rewarder
	onChamReward(uint256, address, address, uint256, uint256) => NONDET
...
rule monotonicityOfAccChamPerShare(uint256 pid, method f) {
	env e;

	uint128 _poolInfoAccChamPerShare = poolInfoAccChamPerShare(pid);

	calldataarg args;
	f(e, args);

	uint128 poolInfoAccChamPerShare_ = poolInfoAccChamPerShare(pid);

	assert compareUint128(poolInfoAccChamPerShare_, _poolInfoAccChamPerShare);
}
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	uint128 _otherAccChamPerShare = poolInfoAccChamPerShare(otherPid);
	...
	uint128 otherAccChamPerShare_ = poolInfoAccChamPerShare(otherPid);
	uint64 otherLastRewardBlock_ = poolInfoLastRewardBlock(otherPid);
	uint64 otherAllocPoint_ = poolInfoAllocPoint(otherPid);

	assert(_otherAccChamPerShare == otherAccChamPerShare_, "accChamPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require CHAM() != lpToken(pid); // <-- check this again (Nurit)
...
rule chamGivenInHarvestEqualsPendingCham(uint256 pid, address user, address to) {
	env e;

	require to == user && user != currentContract && e.msg.sender == user;
	require champagneToken == CHAM();

	uint256 userChamBalance = champagneToken.balanceOf(e, user);
	uint256 userPendingCham = pendingCham(e, pid, user);

	// Does success return value matters? Check with Nurit
	harvest(e, pid, to);

	uint256 userChamBalance_ = champagneToken.balanceOf(e, user);

	assert(userChamBalance_ == (userChamBalance + userPendingCham),
		   "pending champagne not equal to the champagne given in harvest");
}
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingCham(uint256, address).selector) {
		pendingCham(e, pid, to);
