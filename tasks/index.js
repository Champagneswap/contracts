...
task("masterchef:withdraw", "WineMaker withdraw")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const masterChef = await ethers.getContract("WineMaker")
...
task("bar:enter", "ChampagneCellar enter")
.addParam("amount", "Amount")
...
task("bar:leave", "ChampagneCellar leave")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  const cham = await ethers.getContract("ChampagneToken")
...
task("maker:serve", "ChampagneCellar serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
