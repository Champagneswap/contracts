...
// File @champagneswap/Cellar-sdk/contracts/IBatchFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @champagneswap/Cellar-sdk/contracts/IFlashBorrower.sol@v1.0.1
// License-Identifier: MIT
...
// File @champagneswap/Cellar-sdk/contracts/IStrategy.sol@v1.0.1
// License-Identifier: MIT
...
// File @champagneswap/Cellar-sdk/contracts/ICellarV1.sol@v1.0.1
// License-Identifier: MIT
...
interface ISwapper {
    /// @notice Withdraws 'amountFrom' of token 'from' from the Cellar account for this swapper.
    /// Swaps it for at least 'amountToMin' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the Cellar using a plain ERC20 transfer.
    /// Returns the amount of tokens 'to' transferred to Cellar.
    /// (The Cellar skim function will be used by the caller to get the swapped funds).
    function swap(
        IERC20 fromToken,
        IERC20 toToken,
        address recipient,
    ...
    /// Withdraws 'amountFrom' of token 'from' from the Cellar account for this swapper.
    /// Swaps it for exactly 'exactAmountTo' of token 'to'.
    /// Transfers the swapped tokens of 'to' into the Cellar using a plain ERC20 transfer.
    /// Transfers allocated, but unused 'from' tokens within the Cellar to 'refundTo' (amountFromMax - amountFrom).
    /// Returns the amount of 'from' tokens withdrawn from Cellar (amountFrom).
    /// (The Cellar skim function will be used by the caller to get the swapped funds).
    function swapExact(
        IERC20 fromToken,
        IERC20 toToken,
...
// File contracts/GrapePair.sol
// License-Identifier: UNLICENSED
// Grape Lending Medium Risk

/// @title GrapePair
/// @dev This contract allows contract calls to any contract (except Cellar)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract GrapePairMediumRiskV1 is ERC20, BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    Rebase public totalAsset; // elastic = Cellar shares held by the GrapePair, base = Total fractions held by asset suppliers
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Grape Medium Risk ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    // Settings for the Medium Risk GrapePair
    ...
    constructor(ICellarV1 Cellar_) public {
        Cellar = Cellar_;
        masterContract = this;
        feeTo = msg.sender;
    }
    ...
    function init(bytes calldata data) public payable override {
        require(address(collateral) == address(0), "GrapePair: already initialized");
        (collateral, asset, oracle, oracleData) = abi.decode(data, (IERC20, IERC20, IOracle, bytes));
        require(address(collateral) != address(0), "GrapePair: bad pair");
    ...
    function accrue() public {
        ...
        uint256 fullAssetAmount = Cellar.toAmount(asset, _totalAsset.elastic, false).add(_totalBorrow.elastic);
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, false, exchangeRate), "GrapePair: user insolvent");
    }
    ...
    /// False if tokens from msg.sender in `Cellar` should be transferred.
    function _addTokens(
        IERC20 token,
        uint256 share,
        uint256 total,
        ...
        if (skim) {
            require(share <= Cellar.balanceOf(token, address(this)).sub(total), "GrapePair: Skim too much");
        } else {
            Cellar.transfer(token, msg.sender, address(this), share);
        }
    }
    ...
    function addCollateral(
        ...
        emit LogAddCollateral(skim ? address(Cellar) : msg.sender, to, share);
    }
    ...
    function _removeCollateral(address to, uint256 share) internal {
        ...
        Cellar.transfer(collateral, address(this), to, share);
    }
    ...
    function _addAsset(
        ...
        uint256 allShare = _totalAsset.elastic + Cellar.toShare(asset, totalBorrow.elastic, true);
        ...
        emit LogAddAsset(skim ? address(Cellar) : msg.sender, to, share, fraction);
    }
    ...
    function _removeAsset(address to, uint256 fraction) internal returns (uint256 share) {
        ...
        require(_totalAsset.base >= 1000, "Grape: below minimum");
        totalAsset = _totalAsset;
        emit LogRemoveAsset(msg.sender, to, share, fraction);
        Cellar.transfer(asset, address(this), to, share);
    }
    ...
    function _borrow(address to, uint256 amount) internal returns (uint256 part, uint256 share) {
        ...
        share = Cellar.toShare(asset, amount, false);
        Rebase memory _totalAsset = totalAsset;
        require(_totalAsset.base >= 1000, "Grape: below minimum");
        _totalAsset.elastic = _totalAsset.elastic.sub(share.to128());
        totalAsset = _totalAsset;
    ...
    function _repay(
        ...
        uint256 share = Cellar.toShare(asset, amount, true);
        uint128 totalShare = totalAsset.elastic;
        _addTokens(asset, share, uint256(totalShare), skim);
        totalAsset.elastic = totalShare.add(share.to128());
        emit LogRepay(skim ? address(Cellar) : msg.sender, to, amount, part);
    }
    ...
    // Function on Cellar
    uint8 internal constant ACTION_CELLAR_DEPOSIT = 20;
    uint8 internal constant ACTION_CELLAR_WITHDRAW = 21;
    uint8 internal constant ACTION_CELLAR_TRANSFER = 22;
    uint8 internal constant ACTION_CELLAR_TRANSFER_MULTIPLE = 23;
    uint8 internal constant ACTION_CELLAR_SETAPPROVAL = 24;
    ...
    /// @dev Helper function for depositing into `Cellar`.
    function _CellarDeposit(
        bytes memory data,
        uint256 value,
        uint256 value1,
        uint256 value2
        ...
        return Cellar.deposit{value: value}(token, msg.sender, to, uint256(amount), uint256(share));
    }

    /// @dev Helper function to withdraw from the `Cellar`.
    function _CellarWithdraw(
        bytes memory data,
        uint256 value1,
        uint256 value2
    ) internal returns (uint256, uint256) {
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return Cellar.withdraw(token, msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }

    /// @dev Helper function to perform a contract call and eventually extracting revert messages on failure.
    /// Calls to `Cellar` are not allowed for obvious security reasons.
    ...
    function _call(
        ...
        require(callee != address(Cellar) && callee != address(this), "GrapePair: can't call");

        (bool success, bytes memory returnData) = callee.call{value: value}(callData);
        require(success, "GrapePair: call failed");
    ...
    /// Only applicable to `ACTION_CALL`, `ACTION_CELLAR_DEPOSIT`.
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                require((!must_update || updated) && rate > minRate && (maxRate == 0 || rate > maxRate), "GrapePair: rate not ok");
            } else if (action == ACTION_CELLAR_SETAPPROVAL) {
                (address user, address _masterContract, bool approved, uint8 v, bytes32 r, bytes32 s) =
                    abi.decode(datas[i], (address, address, bool, uint8, bytes32, bytes32));
                Cellar.setMasterContractApproval(user, _masterContract, approved, v, r, s);
            } else if (action == ACTION_CELLAR_DEPOSIT) {
                (value1, value2) = _CellarDeposit(datas[i], values[i], value1, value2);
            } else if (action == ACTION_CELLAR_WITHDRAW) {
                (value1, value2) = _CellarWithdraw(datas[i], value1, value2);
            } else if (action == ACTION_CELLAR_TRANSFER) {
                (IERC20 token, address to, int256 share) = abi.decode(datas[i], (IERC20, address, int256));
                Cellar.transfer(token, msg.sender, to, _num(share, value1, value2));
            } else if (action == ACTION_CELLAR_TRANSFER_MULTIPLE) {
                (IERC20 token, address[] memory tos, uint256[] memory shares) = abi.decode(datas[i], (IERC20, address[], uint256[]));
                Cellar.transferMultiple(token, msg.sender, tos, shares);
                ...
                value1 = Cellar.toShare(asset, totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory CellarTotals = Cellar.totals(collateral);
        ...
        require(allBorrowAmount != 0, "GrapePair: all are solvent");
        ...
        uint256 allBorrowShare = Cellar.toShare(asset, allBorrowAmount, true);

        if (!open) {
            // Closed liquidation using a pre-approved swapper for the benefit of the LPs
            require(masterContract.swappers(swapper), "GrapePair: Invalid swapper");

            // Swaps the users' collateral for the borrowed asset
            Cellar.transfer(collateral, address(this), address(swapper), allCollateralShare);
            swapper.swap(collateral, asset, address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = Cellar.balanceOf(asset, address(this)).sub(uint256(totalAsset.elastic));
            ...
            Cellar.transfer(asset, address(this), masterContract.feeTo(), feeShare);
            ...
            Cellar.transfer(collateral, address(this), swapper == ISwapper(0) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(0)) {
                swapper.swap(collateral, asset, msg.sender, allBorrowShare, allCollateralShare);
            }

            Cellar.transfer(asset, msg.sender, address(this), allBorrowShare);
