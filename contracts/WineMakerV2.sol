...
contract WineMakerV2 is BoringOwnable, BoringBatchable {
    ...
    IWineMaker public immutable MASTER_CHEF;
    ...
    uint256 private constant MASTERCHEF_CHAM_PER_BLOCK = 1e20;
    ...
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardBlock, uint256 lpSupply, uint256 accChamPerShare);
    event LogInit();

    /// @param _MASTER_CHEF The ChampagneSwap MCV1 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for CHAM.
    /// Any balance of transaction sender in `dummyToken` is transferred.
    /// The allocation point for the pool on MCV1 is the total allocation point for all pools that receive double incentives.
    /// @param dummyToken The address of the ERC-20 token to deposit into MCV1.
    function init(IERC20 dummyToken) external {
        uint256 balance = dummyToken.balanceOf(msg.sender);
        require(balance != 0, "WineMakerV2: Balance must exceed 0");
    ...
    /// @notice Calculates and returns the `amount` of CHAM per block.
    function chamPerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTERCHEF_CHAM_PER_BLOCK)
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, lpSupply, pool.accChamPerShare);
        }
    }
    ...
    /// @notice Harvests CHAM from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
    function harvestFromWineMaker() public {
